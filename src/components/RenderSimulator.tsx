...
import { Camera, Sliders, RefreshCw } from 'lucide-react';
    ...
    <div className="glass-panel rounded-2xl p-6 md:p-8 border border-white/10 shadow-2xl flex flex-col lg:flex-row-reverse gap-8 items-stretch">
      ...
      <div className="flex-1 flex flex-col justify-between min-h-[300px] lg:min-h-[400px] relative rounded-xl overflow-hidden bg-black/80 border border-white/5 group select-none">
        ...
        <div className="relative z-10 m-4 bg-black/85 backdrop-blur-md p-4 rounded-xl border border-white/15 shadow-2xl flex justify-between items-center flex-wrap gap-3">
          <div>
            <span className="font-mono text-[10px] text-industrial-orange uppercase tracking-widest block">{t.compare.activeScene}</span>
          ...
          <div className="flex gap-4 font-mono text-xs">
            <div>
              <span className="text-[10px] text-on-surface-variant block">{t.compare.exposure}</span>
              <span className="text-white font-bold">EV {(exposure / 10).toFixed(1)}</span>
            </div>
            <div>
              <span className="text-[10px] text-on-surface-variant block">{t.compare.temperature}</span>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center text-xs font-mono">
              <span className="text-on-surface font-semibold">{t.compare.exposureSpeed}</span>
            ...
            <div className="flex justify-between text-[10px] text-on-surface-variant/50 font-mono">
              <span>{t.compare.lowLight}</span>
              <span>{t.compare.overexposed}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center text-xs font-mono">
              <span className="text-on-surface font-semibold">{t.compare.atmosphericKelvin}</span>
            ...
            />
            <div className="flex justify-between text-[10px] text-on-surface-variant/50 font-mono">
              <span>{t.compare.cold}</span>
              <span>{t.compare.warm}</span>
            </div>
          </div>

          {/* Aperture (f-stop) Selection */}
          <div className="flex flex-col gap-2">
            <span className="text-xs font-mono font-semibold text-on-surface">{t.compare.lensAperture}</span>
            ...
            <div className="flex justify-between text-[10px] text-on-surface-variant/50 font-mono">
              <span>{t.compare.shallowFocus}</span>
              <span>{t.compare.infiniteFocus}</span>
            </div>
          </div>
        </div>
